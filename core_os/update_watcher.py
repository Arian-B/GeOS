...
def _record_status(state, filename, status, detail=None):
    entry = state.get(filename, {})
    entry["status"] = status
    entry["detail"] = detail
    try:
        stat = os.stat(os.path.join(INCOMING_DIR, filename))
    ...
    while True:
        ...
        for filename in updates:
            ...
            try:
                ...
                _record_status(state, filename, "staged", f"slot:{slot}")
                _move_to(PROCESSED_DIR, filename, [sig_path, sha_path])
            except Exception as exc:
                _record_status(state, filename, "rejected", str(exc))
