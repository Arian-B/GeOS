...
from PySide6.QtGui import QColor, QTextCharFormat, QPalette
...
class CalendarAppPage(QWidget):
    def __init__(self):
        ...
        self.notify_input = QComboBox()
        for label, minutes in NOTIFY_OPTIONS:
            self.notify_input.addItem(label, minutes)
        self.notify_input.activated.connect(lambda _=None: QTimer.singleShot(0, self.notify_input.hidePopup))
    ...
    def _configure_calendar_widgets(self):
        ...
        for btn, text in ((prev_btn, "<"), (next_btn, ">")):
            if btn is not None:
                btn.setText(text)
    ...
    def highlight_event_dates(self):
        ...
        for year_offset in range(-1, 2):
            year = QDate.currentDate().year() + year_offset
