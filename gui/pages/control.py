...
class ControlPage(QWidget):
    def __init__(self):
        ...
        self.update_status_label = QLabel("Update action: --")

        for lbl in (self.update_current_label, self.update_pending_label, self.update_policy_label, self.update_status_label):
        ...
        self.clear_pending_btn.setFixedHeight(40)
    ...
    def clear_pending_update(self):
        ...
        if removed_any:
            self.update_status_label.setText("Update action: pending/staged update files were cleared.")
        else:
            self.update_status_label.setText("Update action: no pending update files found.")
        self.refresh()
    ...
    def refresh(self):
        ...
        if not control or is_offline(state):
            ...
            if not self.update_status_label.text().startswith("Update action:"):
                self.update_status_label.setText("Update action: --")
        ...
        # Workload controls should remain testable unless the system is in explicit emergency shutdown.
        if emergency:
            for btn in self.workload_buttons.values():
                btn.setEnabled(False)
            for badge in self.workload_status_badges.values():
