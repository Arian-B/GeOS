...
class ReportsAppPage(QWidget):
    ...
    def refresh(self):
        ...
        self.alerts_card.detail.setText(f"{critical} critical, {warn} warnings in the recent alert window.")

        cpu = perf.get("cpu_percent", "--")
        mem = perf.get("memory_percent", "--")
        self.performance_card.value.setText(f"{cpu}% CPU")
        self.performance_card.detail.setText(f"Memory usage: {mem}%")
        ...
        self.runtime_card.value.setText(f"{battery}% battery")
        self.runtime_card.detail.setText(f"Network: {network} | Last update: {state.get('last_updated', '--')}")
