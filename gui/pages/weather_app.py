...
class WeatherAppPage(QWidget):
    ...
    def on_page_deactivated(self):
        ...
        self._stop_weather_thread(wait_ms=9000)

    def _set_offline_state(self):
        self.status.setText("Network offline. Please connect to the internet.")
        ...
        self.ascii_label.setText("  [ offline ]")
    ...
    def _stop_weather_thread(self, wait_ms=9000):
        thread = self._weather_thread
        if thread is None:
            return
        try:
            if thread.isRunning():
                ...
                if not thread.wait(wait_ms):
                    # Last-resort to avoid QThread-destroyed crashes on exit.
                    thread.terminate()
                    thread.wait(1000)
        except RuntimeError:
            # The thread object may already be deleted during shutdown.
            pass
        finally:
            self._reset_worker_handles()
    ...
    def _on_weather_failed(self, message):
        ...
        if any(token in lowered for token in ("offline", "network", "name or service not known", "temporary failure", "timed out")):
            self._set_offline_state()
            return
