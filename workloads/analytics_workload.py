...
SLEEP_BY_INTENSITY = {"LOW": 15, "MEDIUM": 10, "HIGH": 5}
DATA_SIZE_BY_INTENSITY = {"LOW": 100000, "MEDIUM": 200000, "HIGH": 400000}
...
def run():
    ...
    while True:
        ...
        # Adaptive scaling: adjust CPU work and cadence based on intensity.
        intensity = config["analytics_intensity"]
        if intensity not in INTENSITY_SCALE:
            intensity = DEFAULT_CONFIG["analytics_intensity"]
        data_size = DATA_SIZE_BY_INTENSITY[intensity]
        data = [random.random() for _ in range(data_size)]
        _ = sum(data) / len(data)
        time.sleep(SLEEP_BY_INTENSITY[intensity])
